...
contract InvariantUniswapV2PairTest is Test {
    ...
    function invariant_no_token_creation_or_destruction() external {
        ...
        uint256 feeReceiverBalanceToken0 = ERC20(_token0).balanceOf(_uniswapV2PairHandler.feeReceiver());
        uint256 feeReceiverBalanceToken1 = ERC20(_token1).balanceOf(_uniswapV2PairHandler.feeReceiver());
        uint256 skimReceiverBalanceToken0 = ERC20(_token0).balanceOf(_uniswapV2PairHandler.skimReceiver());
        uint256 skimReceiverBalanceToken1 = ERC20(_token1).balanceOf(_uniswapV2PairHandler.skimReceiver());
        uint256 burnReceiverBalanceToken0 = ERC20(_token0).balanceOf(_uniswapV2PairHandler.burnReceiver());
        uint256 burnReceiverBalanceToken1 = ERC20(_token1).balanceOf(_uniswapV2PairHandler.burnReceiver());
        uint256 swapReceiverBalanceToken0 = ERC20(_token0).balanceOf(_uniswapV2PairHandler.swapReceiver());
        uint256 swapReceiverBalanceToken1 = ERC20(_token1).balanceOf(_uniswapV2PairHandler.swapReceiver());

        {
            uint256 sumToken0 =
                deployerBalanceToken0
                + poolBalanceToken0
                + flashBorrowerBalanceToken0
                + feeReceiverBalanceToken0
                + skimReceiverBalanceToken0
                + burnReceiverBalanceToken0
                + swapReceiverBalanceToken0;
            uint256 sumToken1 =
                deployerBalanceToken1
                + poolBalanceToken1
                + flashBorrowerBalanceToken1
                + feeReceiverBalanceToken1
                + skimReceiverBalanceToken1
                + burnReceiverBalanceToken1
                + swapReceiverBalanceToken1;

            assertEq(sumToken0, TOTAL_SUPPLY);
            assertEq(sumToken1, TOTAL_SUPPLY);
        }
    }
    ...
    function invariant_liquidity_provider_share() external {
        ...
        _uniswapV2PairHandler.burn(tokenReceiver);

        assertEq(expectedToken0ReturnAmount, ERC20(_token0).balanceOf(tokenReceiver));
        assertEq(expectedToken1ReturnAmount, ERC20(_token1).balanceOf(tokenReceiver));
    }
